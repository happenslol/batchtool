# compiler to use
COMP = g++
FLAGS = -std=gnu++11
BIN = bin-mingw/batchtool

all: batchtool.o image.o pugixml.o lodepng.o xmlwriter.o base64.o
	$(COMP) $(FLAGS) batchtool.o xmlwriter.o base64.o image.o pugixml.o lodepng.o -o $(BIN)

batchtool.o: src/batchtool.cpp
	$(COMP) $(FLAGS) -c src/batchtool.cpp 

xmlwriter.o: src/xmlwriter.cpp
	$(COMP) $(FLAGS) -c src/xmlwriter.cpp

base64.o: src/base64.cpp
	$(COMP) $(FLAGS) -c src/base64.cpp

image.o: src/image.cpp
	$(COMP) $(FLAGS) -c src/image.cpp 

pugixml.o: lib/pugixml/pugixml.cpp
	$(COMP) $(FLAGS) -c lib/pugixml/pugixml.cpp

lodepng.o: lib/lodepng/lodepng.cpp
	$(COMP) $(FLAGS) -c lib/lodepng/lodepng.cpp

clean:
	rm -rf *o batchtool

